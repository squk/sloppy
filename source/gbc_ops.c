...
void execute_init(gbc_cpu *cpu) { //cli_printl("execute_init");
    a = &cpu->registers.a;
    b = &cpu->registers.b;
    c = &cpu->registers.c;
    d = &cpu->registers.d;
...
void LD_HL_d8(gbc_cpu *cpu) {  write_u8(cpu->mmu,(*h<<8)+*l, read_u8(cpu->mmu, *pc)); *pc+=1; *m=3; };
...
void SBC_mHL(gbc_cpu *cpu) {  u8 ra=*a; u8 rm=read_u8(cpu->mmu,(*h<<8)+*l); *a-=rm; *a-=(*f&0x10)?1:0; *f=(*a<0)?0x50:0x40; *a&=255; if(!*a) *f|=0x80; if((*a^rm^ra)&0x10) *f|=0x20; *m=2; };
